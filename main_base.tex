...
\node[spectral, right=20mm of vin] (fft)
{
$\mathcal{F}(v_l)$\\
FFT
};

\node[spectral, right=of fft] (weight)
{
$R_l \cdot \hat v_l$\\
Spectral weights
};

\node[spectral, right=of weight] (ifft)
{
$\mathcal{F}^{-1}$\\
Inverse FFT
};

\node[local, below=18mm of weight] (conv)
{
$\mathcal{C}_l(v_l)$\\
Local convolution
};

\node[sum, right=18mm of ifft] (sum) {$+$};

\node[act, right=15mm of sum] (sigma)
{
$\sigma$
};

\node[block, right=15mm of sigma] (vout)
{
$v_{l+1}$
};
...
\draw[arr] (vin) -- (fft);
\draw[arr] (fft) -- (weight);
\draw[arr] (weight) -- (ifft);
\draw[arr] (ifft) -- (sum);

\draw[arr] (vin.south) |- (conv.west);
\draw[arr] (conv.east) -- (sum.south);
...
\footnotesize
\begin{itemize}
    \item $\mathcal{F}$ --- Fourier transform
    \item $\mathcal{F}^{-1}$ --- inverse Fourier transform
    \item $R_l$ --- trainable spectral weights
    \item $\mathcal{C}_l$ --- local convolution branch
...
\begin{block}{Channel normalization}
\[
\tilde{s}_{i,c}=
\frac{s_{i,c}-\mu_c}{\sigma_c}
\]
\end{block}

\footnotesize
Normalization statistics are computed on the training set and reused during validation and inference.

\vspace{2mm}

\scriptsize
\begin{itemize}
    \item $\mathcal{L}_{MSE}$ --- mean squared error
    \item $\mathcal{L}_{rel}$ --- relative error
    \item $\lambda_{MSE},\lambda_{rel}$ --- loss weights
    \item $\mu_c$ --- channel mean
    \item $\sigma_c$ --- channel standard deviation
\end{itemize}
...
\footnotesize
FNO provides a neural-operator framework for field prediction, but the current implementation still requires output-scale recalibration before physically reliable interpretation.
\end{alertblock}
...
\begin{itemize}
    ...
    \item channel-wise error analysis
    \item travel-time consistency checks
\end{itemize}
