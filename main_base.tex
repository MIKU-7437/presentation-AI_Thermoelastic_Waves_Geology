  ...
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        \node[box] (inp) {Input layer\\$10$ features};
        \node[box, below=of inp] (h1) {Linear $10 \rightarrow 192$\\Tanh};
        \node[box, below=of h1] (hrep) {$5 \times$ hidden blocks\\Linear $192 \rightarrow 192$ + Tanh};
        \node[box, below=of hrep] (out) {Output layer\\Linear $192 \rightarrow 4$};
        \draw[arrow] (inp) -- (h1);
        \draw[arrow] (h1) -- (hrep);
        \draw[arrow] (hrep) -- (out);
      \end{tikzpicture}
    ...
    \begin{column}{0.40\textwidth}
      ...
      Temperature is predicted in kelvin; displacement is predicted in three
      components. Inputs and supervised outputs are standardized. Velocity, stress, and strain are derived through autograd during residual and velocity-consistency evaluation.}
    \end{column}
  \end{columns}
\end{frame}
