...
\begin{frame}{PINN Input and Architecture}
  \begin{block}{Public neural contract}
    \[
      [x,y,z,t,E,\nu,\rho,\alpha,k,C_p]\rightarrow[T,u,v,w]
    \]
  \end{block}
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        head/.style={draw=AcademicNavy, fill=white, rounded corners=1pt,
          align=center, inner sep=2.5pt, minimum height=6mm},
        arrow/.style={-{Latex[length=2mm]}, thick, draw=AcademicBlue}
      ]
        \node[box] (coord) {coordinates\\$x,y,z,t$};
        \node[box, below=of coord] (mat) {material\\$E,\nu,\rho,\alpha,k,C_p$};
        \node[box, right=of coord] (ce) {coordinate\\encoder};
        \node[box, right=of mat] (me) {material\\encoder};
        \coordinate (mid) at ($(ce)!0.5!(me)$);
        \node[box, right=8mm of mid] (fusion) {fusion};
        \node[box, right=of fusion] (trunk) {residual\\trunk};
        \node[head, right=of trunk, yshift=5mm] (thead) {$T$ head};
        \node[head, right=of trunk, yshift=-5mm] (uhead) {$u,v,w$ head};
        \draw[arrow] (coord) -- (ce);
        \draw[arrow] (mat) -- (me);
        \draw[arrow] (ce) -- (fusion);
        \draw[arrow] (me) -- (fusion);
        \draw[arrow] (fusion) -- (trunk);
        \draw[arrow] (trunk) -- (thead);
        \draw[arrow] (trunk) -- (uhead);
      \end{tikzpicture}
      }

      \vspace{1mm}
      \begin{block}{Architecture variants}
        \scriptsize
        Baseline: $X\rightarrow\mathrm{MLP}_{\theta}(X)\rightarrow\widehat{Y}$.
        Improved route: separate coordinate/material encoders, fused residual
        trunk, and split temperature/displacement heads.
      \end{block}
    \end{column}
    \begin{column}{0.38\textwidth}
      \begin{block}{Notation and units}
        \scriptsize
        \begin{tabular}{@{}ll@{}}
          $x,y,z$ & coordinates [m]\\
          $t$ & time [s]\\
      ...
      \end{block}
      \scriptsize
      Stress and strain are not direct network outputs here; they are recovered
      from spatial derivatives when physics residuals are evaluated.
    \end{column}
  \end{columns}
\end{frame}
...
\node[smallbox, below=1.2cm of spec, xshift=-2.7cm] (fft) {FFT};
\node[smallbox, right=of fft] (modes) {Retained modes\\+ weights};
\node[smallbox, right=of modes] (ifft) {Inverse FFT};
...
\draw[dashed, ->] (spec.south) -- ++(0,-0.3) -| (fft.north);
\draw[dashed, ->] (conv.north) |- (spec.south);
...
\begin{frame}{MeshGraphNet: Message Passing Block}
  ...
  \vskip 4pt
  \scriptsize
  Here \(\phi_e\) and \(\phi_v\) are MLPs with Linear layers, LayerNorm,3на 
...
\begin{frame}{Web Application}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.34\textwidth}
      \begin{block}{Implemented MVP}
        \footnotesize
        A local web workspace connects the frontend, backend orchestrator,
        material catalog, and model services under one prediction contract.
      \end{block}
      \vspace{1mm}
      \footnotesize
      \begin{itemize}
        \item Select geological medium and model route:
              MeshGraphNet, FNO, Transformer, or PINN.
        \item Configure planar source, probe, and observation time.
        \item Inspect normalized thermal, mechanical, directional, and
              temporal response.
      \end{itemize}
      \vspace{1mm}
    \end{column}
    \begin{column}{0.64\textwidth}
      ...
      \vspace{1mm}
      {\scriptsize UI screenshots from the local directional prediction
      workspace: landing view, source--probe geometry, and prediction result.}
    \end{column}
  \end{columns}
\end{frame}
